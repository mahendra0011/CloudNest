...
import {
  User, Lock, Bell, Mail, CheckCircle2, Loader2, AlertCircle, Save, Eye, EyeOff, FileText, HardDrive,
} from 'lucide-react';
import { Button } from '../../components/ui/button.jsx';

const DEMO_USER = { name: 'Demo Admin', email: 'demo-admin@example.com' };
const DEMO_SETTINGS = { maxFileSize: 250, allowedTypes: ['images','documents','videos','audio','archives'], notificationsEnabled: true, sendUploadEmail: true };
...
export function DemoSettingsTab() {
  ...
  const handleUpdateName = () => {
    ...
    setNameMsg(null);
    setTimeout(() => {
      setNameMsg({ type: 'success', text: 'Name updated successfully! (Demo)' });
      setNameSaving(false);
  ...
  const handleChangePassword = () => {
    ...
    setPassMsg(null);
    setTimeout(() => {
      setPassMsg({ type: 'success', text: 'Password changed successfully! (Demo)' });
      setCurrentPassword('');
      setNewPassword('');
      setConfirmPassword('');
      setPassSaving(false);
