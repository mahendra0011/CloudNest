...
import {
  Search, Calendar, ChevronDown, RefreshCw, X, Eye, Download, Trash2,
  ChevronLeft, ChevronRight, Square, CheckSquare, ArrowUpDown, HardDrive,
  Upload, AlertCircle, File as FileIcon, FileImage, FileArchive, Video,
  Loader2, Inbox, ExternalLink, AlertTriangle, CheckCircle, Filter,
  Image, Film, Archive, FileText, Table as TableIcon, Presentation,
  Code, Upload as UploadIcon, FileWarning, FileSpreadsheet
} from 'lucide-react';
...
───────────────────────────────────────────── */
const DEMO_FILES = [
  { id: 'demo-1', originalName: 'demo_invoice.pdf', mimeType: 'application/pdf', size: 245760, category: 'document', user: { name: 'Demo User 1', email: 'demo-user1@example.com' }, drive: { name: 'Demo Primary Drive' }, webViewLink: '#', createdAt: new Date(Date.now() - 1000 * 60 * 60 * 24 * 3), driveFileId: 'demo-gfile-1' },
  { id: 'demo-2', originalName: 'demo_banner.png', mimeType: 'image/png', size: 1048576, category: 'image', user: { name: 'Demo User 2', email: 'demo-user2@example.com' }, drive: { name: 'Demo Primary Drive' }, webViewLink: '#', createdAt: new Date(Date.now() - 1000 * 60 * 60 * 24 * 5), driveFileId: 'demo-gfile-2' },
  { id: 'demo-3', originalName: 'demo_video.mp4', mimeType: 'video/mp4', size: 52428800, category: 'video', user: { name: 'Demo User 3', email: 'demo-user3@example.com' }, drive: { name: 'Demo Backup Drive' }, webViewLink: '#', createdAt: new Date(Date.now() - 1000 * 60 * 60 * 24 * 7), driveFileId: 'demo-gfile-3' },
  { id: 'demo-4', originalName: 'demo_backup.zip', mimeType: 'application/zip', size: 104857600, category: 'archive', user: { name: 'Demo User 1', email: 'demo-user1@example.com' }, drive: { name: 'Demo Primary Drive' }, webViewLink: '#', createdAt: new Date(Date.now() - 1000 * 60 * 60 * 24 * 10), driveFileId: 'demo-gfile-4' },
  { id: 'demo-5', originalName: 'demo_profile.pdf', mimeType: 'application/pdf', size: 1572864, category: 'document', user: { name: 'Demo User 4', email: 'demo-user4@example.com' }, drive: { name: 'Demo Backup Drive' }, webViewLink: '#', createdAt: new Date(Date.now() - 1000 * 60 * 60 * 24 * 12), driveFileId: 'demo-gfile-5' },
  { id: 'demo-6', originalName: 'demo_presentation.pptx', mimeType: 'application/vnd.openxmlformats-officedocument.presentationml.presentation', size: 5242880, category: 'presentation', user: { name: 'Demo User 2', email: 'demo-user2@example.com' }, drive: { name: 'Demo Primary Drive' }, webViewLink: '#', createdAt: new Date(Date.now() - 1000 * 60 * 60 * 24 * 2), driveFileId: 'demo-gfile-6' },
  { id: 'demo-7', originalName: 'demo_spreadsheet.xlsx', mimeType: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet', size: 1048576, category: 'spreadsheet', user: { name: 'Demo User 4', email: 'demo-user4@example.com' }, drive: { name: 'Demo Backup Drive' }, webViewLink: '#', createdAt: new Date(Date.now() - 1000 * 60 * 60 * 24 * 8), driveFileId: 'demo-gfile-7' },
  { id: 'demo-8', originalName: 'demo_script.js', mimeType: 'text/javascript', size: 8192, category: 'code', user: { name: 'Demo User 3', email: 'demo-user3@example.com' }, drive: { name: 'Demo Backup Drive' }, webViewLink: '#', createdAt: new Date(Date.now() - 1000 * 60 * 60 * 24 * 1), driveFileId: 'demo-gfile-8' },
];

const DEMO_DRIVES = [
  { _id: 'demo-drive-1', name: 'Demo Primary Drive' },
  { _id: 'demo-drive-2', name: 'Demo Backup Drive' },
];
...
───────────────────────────────────────────── */
const STATUS_STYLES = {
  uploaded: { dot: 'bg-emerald-500', text: 'text-emerald-600 dark:text-emerald-400', bg: 'bg-emerald-50 dark:bg-emerald-950/30', label: 'Uploaded' },
  processing: { dot: 'bg-amber-400', text: 'text-amber-600 dark:text-amber-400', bg: 'bg-amber-50 dark:bg-amber-950/30', label: 'Processing' },
  error: { dot: 'bg-red-500', text: 'text-red-600 dark:text-red-400', bg: 'bg-red-50 dark:bg-red-950/30', label: 'Failed' },
};
...
───────────────────────────────────────────── */
const FILE_META = {
  image: { bg: 'bg-gradient-to-br from-violet-500 to-purple-600', icon: Image, label: 'IMG' },
  video: { bg: 'bg-gradient-to-br from-rose-500 to-pink-600', icon: Film, label: 'VID' },
  archive: { bg: 'bg-gradient-to-br from-amber-500 to-orange-600', icon: Archive, label: 'ARC' },
  document: { bg: 'bg-gradient-to-br from-blue-500 to-indigo-600', icon: FileText, label: 'DOC' },
  spreadsheet: { bg: 'bg-gradient-to-br from-emerald-500 to-teal-600', icon: TableIcon, label: 'XLS' },
  presentation: { bg: 'bg-gradient-to-br from-orange-500 to-red-600', icon: Presentation, label: 'PPT' },
  pdf: { bg: 'bg-gradient-to-br from-red-500 to-rose-600', icon: FileWarning, label: 'PDF' },
  text: { bg: 'bg-gradient-to-br from-slate-500 to-slate-600', icon: FileSpreadsheet, label: 'TXT' },
  code: { bg: 'bg-gradient-to-br from-cyan-500 to-blue-600', icon: Code, label: 'CODE' },
  other: { bg: 'bg-gradient-to-br from-slate-400 to-slate-500', icon: FileIcon, label: 'FILE' },
};
...
/* ── Dropdown component ── */
function Dropdown({ label, icon: Icon, value, options, onChange, className }) {
  ...
  return (
    <div ref={ref} className={cn('relative', className)}>
      ...
      {open && (
        <div className="absolute top-full left-0 mt-1 z-40 min-w-[200px] rounded-lg border shadow-lg bg-white border-slate-200 dark:bg-slate-900 dark:border-slate-700 overflow-hidden">
...
/* ══════════════════════════════════════════
    Main Component
═══════════════════════════════════════════ */
export default function DemoUploadManagementtab() {
  const files = DEMO_FILES;
  const drives = DEMO_DRIVES;
  const selectedDrives = drives;
  const selectedDriveIds = drives.map(d => d._id);
  ...
  React.useEffect(() => { setPage(1); setSelectedIds([]); }, [searchQuery, typeFilter, dateFilter, dateRange, sortBy]);
  ...
  const drivesToShow = selectedDrives;
      ...
      <div className="mb-5">
        <div className="flex items-center gap-3 flex-wrap">
          ...
          {dateFilter === 'custom' && (
            <div className="flex items-center gap-2">
              ...
              <input type="date" value={dateRange.to} onChange={(e) => setDateRange((p) => ({ ...p, to: e.target.value }))}
                className="rounded-lg border border-slate-200 bg-white px-2 py-2 text-xs text-slate-700 focus:outline-none focus:ring-2 focus:ring-blue-500/20 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-300" />
              <button onClick={() => setPage(1)} className="rounded-lg bg-blue-600 px-3 py-2 text-xs font-bold text-white hover:bg-blue-700 transition-colors">
                Apply
              </button>
            </div>
          )}
          ...
          <button
            ...
            className="inline-flex items-center gap-2 px-3.5 py-2.5 rounded-lg border border-slate-200 bg-white text-sm text-slate-600 hover:bg-slate-50 hover:border-slate-300 transition-all shrink-0 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-400 dark:hover:bg-slate-800 dark:hover:border-slate-600"
            title="Refresh"
          >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-lg rounded-2xl border border-slate-200 bg-white p-6 shadow-2xl dark:border-slate-700 dark:bg-slate-900">
              ...
              >
                ...
                {uploadFile ? (
                  <>
                    ...
                    <button onClick={(e) => { e.stopPropagation(); setUploadFile(null); }} className="text-slate-400 hover:text-red-500 transition-colors">
                      <X className="h-4 w-4" />
                    </button>
                  </>
                ) : (
            ...
            <div className="mb-4">
              ...
              <input type="text" placeholder="e.g. Rajesh Kumar" value={uploadUserName} onChange={(e) => setUploadUserName(e.target.value)} disabled={uploading}
            ...
            <div className="mb-4">
              ...
              <input type="email" placeholder="e.g. rajesh@example.com" value={uploadUserEmail} onChange={(e) => setUploadUserEmail(e.target.value)} disabled={uploading}
                className="w-full px-3.5 py-2.5 rounded-lg border border-slate-200 bg-white text-sm text-slate-700 placeholder-slate-400 outline-none focus:border-blue-400 focus:ring-2 focus:ring-blue-500/20 transition-all disabled:opacity-60 dark:border-slate-700 dark:bg-slate-950 dark:text-slate-300 dark:placeholder-slate-500" />
            ...
            <div className="mb-5">
              ...
              <select value={uploadDriveId} onChange={(e) => setUploadDriveId(e.target.value)} disabled={uploading}
